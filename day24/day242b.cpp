...
bool Circuit::fulladder(wire *w, int op)
{
    if(w->op == NOOP)
        return true;
    if(w->op == AND)
        return (w->in1->label[0] == 'x' || w->in1->label[0] == 'y') ||
                (fulladder(w->in1, AND) && fulladder(w->in2, OR)) ||
                (fulladder(w->in1, OR) && fulladder(w->in2, AND));

    if(w->op == XOR)
        return (w->in1->label[0] == 'x' || w->in1->label[0] == 'y') ||
                (fulladder(w->in1, OR) && fulladder(w->in2, XOR)) ||
                (fulladder(w->in1, XOR) && fulladder(w->in2, OR));
    if(w->op == OR)
        return fulladder(w->in1, AND) && fulladder(w->in2, AND);
}
...
/*     set<string> errors;
   ...
   string bad[] = {"gbf","nbf","nwr","pdk","shs","z05","z09","z15"};
    ...
    wire *z = wires.find("z45");
    bool ok = fulladder(z, OR);
    cout << ok << endl;
}   
